...
{
    public float maxXVel, maxEmission, stormForceMax, maxVol, minVol, colorChanged;
    ...
    private IEnumerator RampUp()
    {
        ...
        while (ramping)
        {
            ...
            foreach ( SpriteRenderer child in bg.GetComponentsInChildren<SpriteRenderer>())
            {
                float r = Mathf.Lerp(1, colorChanged, t);
                float g = Mathf.Lerp(1, colorChanged, t);
                float b = Mathf.Lerp(1, colorChanged, t);

                child.color = new Color(r, b, g, 1);
            }

            storm.forceMagnitude = Mathf.Lerp(0, stormForceMax, t);
            audioS.volume = Mathf.Lerp(minVol, maxVol, t);
